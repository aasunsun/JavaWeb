...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            用户名：<input name="pname" id="pname">&nbsp;&nbsp;&nbsp;
            用户等级：<select name="typeid" id="typeid">
        ...
        <c:choose>
            <c:when test="${info.list.size()!=0}">
                ...
                <div id="top">
                    ...
                    <a href="${pageContext.request.contextPath}/admin/addmyusers.jsp">

                        <input type="button" class="btn btn-warning" id="btn1"
                               value="添加用户">
                    </a>
                    <input type="button" class="btn btn-warning" id="btn1"
                           value="批量删除用户" onclick="deleteBatch()">
                </div>
                <!--显示分页后的商品-->
                <div id="middle">
                    <table class="table table-bordered table-striped">
                        ...
                        <c:forEach items="${info.list}" var="p">
                            <tr>
                                <td valign="center" align="center"><input type="checkbox" name="ck" id="ck" value="${p.uid}" onclick="ckClick()"></td>
                                <td>${p.uname}</td>
                                <td>${p.ustatus}</td>
                                <td>${p.ulevel}</td>
                                <td>${p.score}</td>
                                ...
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="one(${p.uid},${info.pageNum})">编辑
                                    </button>
            ...
            <c:otherwise>
                <div>
                    <h2 style="width:1200px; text-align: center;color: orangered;margin-top: 100px">未找到该用户！</h2>
                </div>
            </c:otherwise>
        </c:choose>
    </div>
...
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        //异步ajax分页请求
        $.ajax({
            url:"${pageContext.request.contextPath}/myusers/ajaxSplit.action",
            data:{"page":page},
            type:"post",
            success:function () {
                //重新加载分页显示的组件table
                $("#table").load("http://localhost:9999/admin/myusers.jsp #table");
            }
        });
    }
